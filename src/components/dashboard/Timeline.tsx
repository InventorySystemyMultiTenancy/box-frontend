...
export default function Timeline({ events, justArrivedId }: { events: TimelineEvent[]; justArrivedId?: string | null }) {
  if (events.length === 0) {
    ...
    <div className={styles.timeline}>
      {events.map((event) => (
        ...
        >
          ...
          {event.description && <div className={styles.tlSub}>{event.description}</div>}
        </div>
      ))}
    </div>
  );
